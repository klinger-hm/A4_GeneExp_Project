...
All models adjust for baseline age, sex, BMI, and cohort (A4 vs LEARN).
...
There are `r length(subset(pTAUTable_GeneAb, fdr_p < 0.05)$gene)` significant gene that survives FDR correction.
...
```{r}
BL_pTAUTable_GeneSexAb <- merge(BL_pTAUTable_GeneSexAb, gene_names, by = "gene", all.x = TRUE)

BL_pTAUTable_GeneSexAb <- BL_pTAUTable_GeneSexAb %>%
  mutate(Sig = case_when(
    p < 0.05 ~ T,
    TRUE ~ F
  ))
...
There are `r length(subset(BL_pTAUTable_GeneSexAb, fdr_p < 0.05)$gene)` significant gene that survives FDR correction.

### Volcano Plot

```{r}

sig_bon <- -log10(0.05/length(unique(BL_pTAUTable_GeneSexAb$gene))) #Bonferroni correction for plotting

ggplot(BL_pTAUTable_GeneSexAb, aes(x = beta, y = -log10(p), color = Sig)) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), color = "red", linetype = "dashed", size = 1.2) +
  geom_hline(yintercept = sig_bon, color = "blue", linetype = "dashed", size = 1) +
  theme(legend.position="none") +
...
```{r}
datatable(subset(BL_pTAUTable_GeneSexAb, fdr_p < 0.05),
          filter = "top",
          rownames = FALSE,
          extensions = c('Buttons'),
          options = list(dom = 'Btip',
